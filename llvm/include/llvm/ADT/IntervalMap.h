...
/// Pair of node indices used when splitting or balancing B+-tree nodes.
...
class NodeBase {
public:
  /// Maximum number of elements stored in this node.
  static constexpr unsigned Capacity = N;

  /// Primary payload array (interval bounds or child NodeRefs).
  T1 first[N];
  /// Secondary payload array (mapped values or stop keys).
  T2 second[N];
...
/// Cache-line sizing constants used when choosing IntervalMap node capacities.
enum CacheSizing {
  ...
  /// log2 of the assumed cache line size in bytes.
  Log2CacheLine = 6,
  /// Assumed cache line size in bytes.
  CacheLineBytes = 1 << Log2CacheLine,
  /// Preferred node allocation size (three cache lines).
  DesiredNodeBytes = 3 * CacheLineBytes
};

/// Compute leaf and branch node capacities and allocator sizing for key/value types.
template <typename KeyT, typename ValT>
struct NodeSizer {
  /// Leaf capacity derived from key/value sizes and cache-line targets.
  enum LeafSizing {
    ...
    /// Ideal leaf capacity if it fits three cache lines.
    DesiredLeafSize = DesiredNodeBytes /
      static_cast<unsigned>(2*sizeof(KeyT)+sizeof(ValT)),
    /// Minimum leaf capacity required by B+-tree balancing.
    MinLeafSize = 3,
    /// Actual leaf branching factor used for this key/value pair.
    LeafSize = DesiredLeafSize > MinLeafSize ? DesiredLeafSize : MinLeafSize
  };

  /// NodeBase layout used for leaves of capacity LeafSize.
  using LeafBase = NodeBase<std::pair<KeyT, KeyT>, ValT, LeafSize>;

  /// Allocation unit and branch capacity derived from LeafBase size.
  enum AllocSizing {
    // Now that we have the leaf branching factor, compute the actual allocation
    // unit size by rounding up to a whole number of cache lines.
    /// Bytes allocated per node, rounded up to a whole number of cache lines.
    AllocBytes = (sizeof(LeafBase) + CacheLineBytes-1) & ~(CacheLineBytes-1),

    // Determine the branching factor for branch nodes.
    /// Branching factor for interior (branch) nodes.
...
/// Polymorphic reference to a leaf or branch node plus its element count.
class NodeRef {
...
/// Leaf B+-tree node storing up to N coalesced key intervals and values.
template <typename KeyT, typename ValT, unsigned N, typename Traits>
...
/// Root-to-leaf path used to navigate an IntervalMap tree.
class Path {
...
public:
  /// Construct an empty map that allocates nodes from \p a.
  /// @param a Allocator used for all node storage.
  explicit IntervalMap(Allocator &a) : allocator(&a) {
    new (&rootLeaf()) RootLeaf();
  }

  ///@{
  /// Copy-construct by allocating with \p RHS's allocator then assigning.
  ///
  ...
  /// Copy-assign by clearing this map and inserting all intervals from \p RHS.
  /// @param RHS Map to copy from; its allocator is adopted.
  /// @return This map after the copy.
  ...
  /// Move-construct by transferring the tree and allocator from \p RHS.
  /// @param RHS Map to move from; left with an empty inline root.
  ...
  /// Move-assign by transferring the tree and allocator from \p RHS.
  /// @param RHS Map to move from; left with an empty inline root.
  /// @return This map after the move.
