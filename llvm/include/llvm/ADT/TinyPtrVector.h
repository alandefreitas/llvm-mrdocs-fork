...
class TinyPtrVector {
public:
  /// Heap vector type used once more than one element is stored.
  using VecTy = SmallVector<EltTy, 4>;
  /// Element pointer type stored in the vector.
  ...
  /// Tagged union holding either a single element or a pointer to \c VecTy.
...
public:
  ...
  /// Destroy any heap-allocated multi-element storage.
  ~TinyPtrVector() {
    if (VecTy *V = dyn_cast_if_present<VecTy *>(Val))
      delete V;
  }

  /// Copy-construct from \p RHS, cloning heap storage when present.
  ...
  /// Move-construct from \p RHS, leaving \p RHS empty.
  ...
  /// Construct from the elements in initializer list \p IL.
  /// @param IL Elements to copy into the vector.
  ...
  /// Construct a vector of \p Count copies of \p Value.
  /// @param Count Number of elements.
  /// @param Value Element pointer to repeat (must be non-null if Count > 0).
  ...
  /// Mutable pointer iterator over elements.
  using iterator = EltTy *;
  /// Const pointer iterator over elements.
  ...
  /// Append \p NewVal; must be non-null.
  /// @param NewVal Element pointer to append.
  ...
  /// Remove the last element; the vector must be non-empty.
  void pop_back() {
  ...
  /// Remove all elements, retaining any heap vector for reuse.
  void clear() {
    // If we have a single value, convert to empty.
    if (isa<EltTy>(Val)) {
      Val = EltTy();
  ...
  /// Insert the range [\p From, \p To) before \p I.
  /// @param I Insertion position.
