...
/// Machine IR instruction type used when printing MIR uniformity results.
class MachineInstr;
...
public:
  ...
  /// Cycle-info type instantiated for this context.
  using CycleInfoT = GenericCycleInfo<ContextT>;
  /// Const iterator over blocks in the modified post-order.
...
//
/// Sync-dependence analysis that finds join points and divergent cycle exits
/// caused by divergent terminators.
...
public:
  ...
  /// Function type from the SSA context.
  using FunctionT = typename ContextT::FunctionT;
  /// Mutable value reference type from the SSA context.
  ...
  /// Set of const basic-block pointers.
  using ConstBlockSet = SmallPtrSet<const BlockT *, 4>;
  /// Modified post-order type for this context.
  ...
  struct DivergenceDescriptor {
    /// Join points reachable by disjoint paths from the divergent terminator.
    ConstBlockSet JoinDivBlocks;
    /// Cycle exits that are divergent relative to the terminator.
    ConstBlockSet CycleDivBlocks;
  };

  /// Propagator that labels blocks along diverged paths.
...
public:
  ...
  using FunctionT = typename ContextT::FunctionT;
  using ValueRefT = typename ContextT::ValueRefT;
  /// Const value reference type from the SSA context.
...
public:
  ...
  /// Modified post-order type for this context.
  using ModifiedPO = ModifiedPostOrder<ContextT>;
  /// Sync-dependence analysis type for this context.
  using SyncDependenceAnalysisT = GenericSyncDependenceAnalysis<ContextT>;
  /// Divergence descriptor produced by the sync-dependence analysis.
  using DivergenceDescriptorT =
      typename SyncDependenceAnalysisT::DivergenceDescriptor;

  /// Modified post-order used to walk blocks during propagation.
  const ModifiedPO &CyclePOT;
  /// Dominator tree for the function under analysis.
  const DominatorTreeT &DT;
  /// Cycle information for the function under analysis.
  const CycleInfoT &CI;
  /// Divergent terminator block that seeds this propagation.
  const BlockT &DivTermBlock;
  /// SSA context used for printing and graph queries.
  const ContextT &Context;
  ...
  /// Indices of blocks whose labels changed and need another pass.
  SparseBitVector<> FreshLabels;

  // divergent join and cycle exit descriptor.
  /// Accumulated join points and divergent cycle exits for this seed.
  ...
  //                          itself
  /// Per-block labels used while discovering divergent joins.
...
/// Drain the worklist, propagating data and control divergence to a fixed point.
template <typename ContextT>
