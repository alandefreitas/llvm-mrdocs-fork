...
/// Trait that yields a pointer to const \c T.
template <typename T> struct make_const_ptr {
  /// Pointer to a const-qualified \c T.
  using type = std::add_pointer_t<std::add_const_t<T>>;
};

/// Trait that yields a const lvalue reference to \c T.
template <typename T> struct make_const_ref {
  /// Const lvalue reference to \c T.
...
/// True if \c T and every type in \c Ts are the same.
template <typename T = void, typename... Ts>
...
///
/// Derived iterators must provide a `mapElement` method that defines how to map
/// a value of the iterator to the provided reference type.
...
class mapped_iterator_base
    : public iterator_adaptor_base<
          ...
          std::remove_reference_t<ReferenceTy> *, ReferenceTy> {
public:
  /// Base type for derived mapped iterator implementations.
...
protected:
  /// End iterator for the filtered range.
  WrappedIteratorT End;
  /// Predicate used to filter elements.
  PredicateT Pred;

  /// Advance until the predicate accepts the current element or the end is reached.
  void findNextValid() {
    while (this->I != End && !Pred(*this->I))
      BaseT::operator++();
  }

  /// Default-construct an empty filter iterator.
...
public:
  /// Default-construct an empty forward filter iterator.
...
///
/// Ranges with offsetable bases should derive from indexed_accessor_range_base
...
struct on_first {
  /// Binary function applied to the first component of each pair.
  FuncTy func;
...
/// Sort POD-like elements with \c qsort to reduce template bloat.
///
/// This sorts an array with the specified start and end extent. This is just
/// like std::sort, except that it calls qsort instead of using an inlined
/// template. qsort is slightly slower than std::sort, but most sorts are not
/// performance critical in LLVM and std::sort has to be template instantiated
/// for each type, leading to significant measured code bloat. This function
/// should generally be used instead of std::sort where possible.
///
...
/// Hash functor for \c std::pair values.
