...
public:
  /// Construct an empty queue with comparator \p compare and container \p sequence.
  explicit PriorityQueue(const Compare &compare = Compare(),
                         const Sequence &sequence = Sequence())
    : std::priority_queue<T, Sequence, Compare>(compare, sequence)
  {}

  /// Construct a queue from the range [\p begin, \p end) with optional comparator
  /// and initial container.
