...
public:
  ...
  /// Construct a vector that allocates pages from the external allocator \p A.
  /// @param A Non-null bump-pointer allocator that outlives this vector.
  explicit PagedVector(BumpPtrAllocator *A) : Allocator(A, false) {
    assert(A && "Allocator cannot be nullptr");
  }

  /// Destroy materialized pages and, if owned, the internal allocator.
  ~PagedVector() {
    clear();
  ...
  public:
    ...
    /// Construct an iterator positioned at \p ElementIdx in \p PV.
    /// @param PV Vector whose materialized pages are being walked.
    /// @param ElementIdx Logical element index (may be size() for end).
