...
///
/// Given input such as \c {{0, 1}, {2}, {3, 4}}, yields every tuple formed by
/// picking one choice per variable: \c {0, 2, 3}, \c {0, 2, 4}, \c {1, 2, 3},
/// \c {1, 2, 4}. The outer range is the variable space; each inner range is the
/// set of choices for that variable.
...
public:
  /// Construct a generator over the given per-variable choice lists.
  /// @param VariablesChoices_ Non-empty list of non-empty choice ranges.
  ...
  /// Invoke \p Callback for every combination in the choice space.
  /// @param Callback Called with each combination; return true to abort early.
