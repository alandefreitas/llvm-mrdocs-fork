...
/// Befriended by DenseMapBase so it can use bucket-relocation erase callbacks.
/// ValueHandleBase is the only caller that caches pointers into bucket arrays.
...
public:
  ...
  /// Remove all entries from the map, possibly shrinking storage if capacity
  /// is large relative to size.
  void clear() {
    incrementEpoch();
    if (getNumEntries() == 0)
      return;
  ...
  /// Find an entry using an alternate lookup key type.
  ///
  /// DenseMapInfo must supply getHashValue(LookupKeyT) and
  /// isEqual(LookupKeyT, KeyT) for each key type used.
  /// @param Val Lookup key, which may differ from KeyT.
  ...
  /// Const overload of find_as(); see the non-const overload for details.
  /// @param Val Lookup key, which may differ from KeyT.
  ...
  /// If the key exists, the stored value is left unchanged.
  ...
  /// Insert a key and construct its value in-place if the key is absent.
  /// If the key is already present, the stored value is left unchanged.
  /// @param Key Key to insert, moved from.
...
protected:
  /// Default-construct an empty DenseMapBase; derived types allocate storage.
  DenseMapBase() = default;

  /// Tag type selecting constructors that take an exact bucket count rather
  /// than a reserve-for-N-entries hint.
  struct ExactBucketCount {};

  /// Hot-path snapshot of bucket storage fields.
  ///
  /// Fetching them together lets SmallDenseMap test its Small discriminator
  /// once rather than once per accessor; for plain DenseMap it is three member
  /// loads either way.
  struct Rep {
    const BucketT *Buckets; ///< Bucket array base pointer.
    const UsedT *Used;      ///< Parallel occupancy bit array.
    unsigned NumBuckets;    ///< Number of buckets in the storage array.
  };
...
public:
  ...
  /// Copy-construct a DenseMap by inserting all entries from \p other.
...
///
/// Skips empty slots using the parallel used-bit array and participates in
/// epoch-based invalidation checks via DebugEpochBase::HandleBase.
...
public:
  ...
  /// Reference type for the bucket at the iterator position.
  using value_type = std::conditional_t<IsConst, const Bucket, Bucket>;
  /// Pointer to a bucket (const when iterating a const map).
  using pointer = value_type *;
  /// Reference to the bucket at the iterator position.
...
public:
  ...
  /// Return an iterator to the first occupied bucket, or end if \p IsEmpty.
  /// @param Buckets Bucket array base.
  /// @param Used Parallel occupancy bit array.
  /// @param NumBuckets Number of buckets in the array.
  /// @param IsEmpty Whether the map has no entries.
  /// @param Epoch Epoch tracker for iterator invalidation checks.
  /// @return Iterator to the first entry or past-the-end.
  ...
  /// Return a past-the-end iterator for the bucket range.
  /// @param Buckets Bucket array base.
  /// @param Used Parallel occupancy bit array.
  /// @param NumBuckets Number of buckets in the array.
  /// @param Epoch Epoch tracker for iterator invalidation checks.
  ...
  /// Build an iterator positioned at bucket pointer \p P within \p Buckets.
  /// @param P Pointer to the current bucket.
  /// @param Buckets Base of the bucket array.
  ...
  /// SFINAE'd out for const iterator destinations so it does not replace the
...
/// Return the allocated bucket storage size of \p X in bytes.
/// @param X Map whose bucket and occupancy arrays are measured.
/// @return Number of bytes in the combined bucket and used-bit allocation.
