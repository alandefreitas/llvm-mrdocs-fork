...
public:
  ...
  /// Mutable iterator over elements that share a key.
  using iterator = iterator_base<SparseMultiSet *>;
  /// Const iterator over elements that share a key.
  using const_iterator = iterator_base<const SparseMultiSet *>;

  // Convenience types
  /// Pair of iterators delimiting the equal-range for one key.
  ...
  /// Find an element by its key (const overload).
  ///
  /// @param   Key A valid key to find.
  /// @returns A const iterator to the element identified by key, or end().
