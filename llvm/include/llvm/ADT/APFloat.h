...
enum lostFraction { // Example of truncated bits:
  /// Truncated bits were all zero.
  lfExactlyZero,    // 000000
  /// Truncated bits were less than half an LSB (and not zero).
  lfLessThanHalf,   // 0xxxxx  x's not all zero
  /// Truncated bits were exactly half an LSB.
  lfExactlyHalf,    // 100000
  /// Truncated bits were more than half an LSB.
...
///
/// Holds floating-point semantic enums, status/category codes, and accessors
/// for built-in \c fltSemantics. This class defines no non-static data members.
class APFloatBase {
public:
  /// Unsigned word type used for multi-precision integer parts of a float.
  typedef APInt::WordType integerPart;
  /// Number of bits in one \c integerPart word.
  ...
  /// @{
  /// Identifiers for the built-in floating-point formats APFloat supports.
  enum Semantics {
    /// IEEE 754 binary16 (half precision).
    S_IEEEhalf,
    /// Google brain floating-point format (bfloat16).
    S_BFloat,
    ...
    /// represented with the exponent and mantissa bits set to all 1s.
    S_Float8E5M3FNU,
    /// x87 80-bit double-extended precision format.
    S_x87DoubleExtended,
    /// Sentinel equal to the highest valid \c Semantics enumerator.
...
public:
  ...
  /// representable in semantics \p B.
  ///
  ...
  enum cmpResult {
    /// Left-hand value is ordered less than the right-hand value.
    cmpLessThan,
    /// Values compare equal (including +0 vs -0).
    cmpEqual,
    /// Left-hand value is ordered greater than the right-hand value.
    cmpGreaterThan,
    /// Comparison is unordered (at least one operand is NaN).
    cmpUnordered
  };
  ...
  enum opStatus {
    /// Operation completed without raising an IEEE exception.
    opOK = 0x00,
    /// Invalid operation exception (e.g. 0/0, Inf-Inf).
    opInvalidOp = 0x01,
  ...
  enum fltCategory {
    ...
    fcNaN,
    /// Finite non-zero value (normal or denormal).
    fcNormal,
  ...
  enum uninitializedTag {
    /// Tag selecting the uninitialized APFloat constructor.
    uninitialized
  };

  /// Enumeration of \c ilogb error results.
  enum IlogbErrorKinds {
    /// \c ilogb result for a zero operand.
    IEK_Zero = INT_MIN + 1,
    /// \c ilogb result for a NaN operand.
    IEK_NaN = INT_MIN,
    /// \c ilogb result for an infinity operand.
    IEK_Inf = INT_MAX
  };
...
///
/// Currently used with \c fltNonfiniteBehavior::NanOnly; using a variant other
/// than IEEE while having IEEE non-finite behavior is liable to lead to
/// unexpected results.
...
struct fltSemantics {
  ...
  /// How Inf and NaN are encoded for this format.
  fltNonfiniteBehavior nonFiniteBehavior = fltNonfiniteBehavior::IEEE754;

  /// How NaN bit patterns are encoded when non-finite behavior is NanOnly.
...
public:
  ...
  /// Flip the sign bit of this value.
  void changeSign() { APFLOAT_DISPATCH_ON_SEMANTICS(changeSign()); }
  /// Force the sign bit clear (make the value non-negative).
  void clearSign() {
    if (isNegative())
      changeSign();
  }
  ...
  ///
  /// Rounding follows \p RM. On invalid operation the result is deterministic:
