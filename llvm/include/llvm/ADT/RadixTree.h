...
public:
  /// Key type stored in each tree entry.
  using key_type = KeyType;
  /// Mapped value type associated with each key.
  using mapped_type = T;
  /// Key/value pair type stored in the tree.
...
public:
  ...
  /// Const iterator over key/value pairs that are prefixes of a query key.
  using const_prefix_iterator = IteratorImpl<const value_type>;

  /// Mutable iterator over all stored key/value pairs.
  using iterator = typename ContainerType::iterator;
  /// Const iterator over all stored key/value pairs.
