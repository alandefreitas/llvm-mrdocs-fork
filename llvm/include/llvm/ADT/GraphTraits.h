...
///
/// Specialize for each graph (CFG, DomTree, …). The primary template is empty
/// except for a deliberate incomplete \c NodeRef that fails compilation when
/// no specialization is visible. May reinterpret the graph via richer
/// \c NodeRef values (see LoopBodyTraits).
...
struct GraphTraits {
  ...
  // file #include'd.
  /// Placeholder that fails compilation unless GraphTraits is specialized.
...
template <class GraphType>
struct Inverse {
  /// The wrapped graph traversed in inverse order.
  const GraphType &Graph;

  /// Wrap graph \p G for inverse-order traversal.
  /// @param G Graph to wrap.
