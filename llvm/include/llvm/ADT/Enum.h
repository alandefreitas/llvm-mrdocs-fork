...
template <typename T, unsigned NumStrs = 1> struct EnumStringDef {
  ...
  /// Enumeration value associated with \ref Names.
  T Value;
};
...
struct EnumStringsStorage {
  /// Compact entries with relative offsets into \ref Strs.
  EnumString<T, NumStrs> Data[N];
  /// Contiguous character storage for all enumerator name strings.
  char Strs[StrLen];
...
public:
  /// Compact entry type stored in this table.
...
/// Deduce EnumStrings template arguments from EnumStringsStorage.
template <typename T, unsigned NumStrs, size_t N, size_t StrLen>
