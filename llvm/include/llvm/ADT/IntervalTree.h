...
protected:
  /// Type of the interval endpoints.
  using PointType = PointT;
  /// Type of the value associated with an interval.
...
public:
  /// Deleted: intervals must be constructed with endpoints and a value.
  IntervalData() = delete;
  /// Construct a closed interval [\p Left, \p Right] mapped to \p Value.
  /// @param Left Left endpoint (must be <= \p Right).
...
//---                          IntervalTree                               ----//
//===----------------------------------------------------------------------===//
/// Trait that is true when \p T is a fundamental type suitable as a point.
template <typename T> using PointTypeIsValid = std::is_fundamental<T>;

/// Trait that is true when \p T is a fundamental or pointer value type.
template <typename T>
...
///
/// Intervals are inserted offline and then built into a tree. Queries return
/// all intervals that contain a given point, optionally sorted by endpoint.
///
/// @tparam PointT Fundamental type used for interval endpoints.
/// @tparam ValueT Fundamental or pointer type stored with each interval.
/// @tparam DataT Interval record type (defaults to IntervalData).
...
public:
  ...
  enum class Sorting {
    /// Sort by ascending left endpoint.
    Ascending,
    /// Sort by descending right endpoint.
    Descending
  };
...
public:
  /// Construct an empty interval tree that allocates nodes from \p NodeAllocator.
  /// @param NodeAllocator Bump allocator for tree nodes.
