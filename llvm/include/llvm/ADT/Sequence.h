...
/// Traits marking whether \p EnumT is safe to iterate with enum_seq helpers.
///
...
template <typename EnumT> struct enum_iteration_traits {
  /// When true, \p EnumT may be iterated by enum_seq without the force tag.
...
struct force_iteration_on_noniterable_enum_t {
  /// Construct the force-iteration tag.
...
/// Half-open or closed integer/enum range yielding successive values of type \p T.
template <typename T> struct iota_range {
  /// Element type produced by the range.
  using value_type = T;
  /// Mutable reference typedef (values are produced by value).
  using reference = T &;
  /// Const reference typedef (values are produced by value).
  using const_reference = const T &;
  /// Forward iterator over values in the range.
  using iterator = detail::SafeIntIterator<value_type, false>;
  /// Const forward iterator over values in the range.
  using const_iterator = iterator;
  /// Reverse iterator over values in the range.
  using reverse_iterator = detail::SafeIntIterator<value_type, true>;
  /// Const reverse iterator over values in the range.
  using const_reverse_iterator = reverse_iterator;
  /// Signed distance between iterators.
  using difference_type = intmax_t;
  /// Unsigned size of the range.
  using size_type = std::size_t;

  /// Construct a range from \p Begin to \p End (inclusive if \p Inclusive).
  /// @param Begin First value in the range.
  /// @param End Past-the-end value, or last value when \p Inclusive is true.
  /// @param Inclusive When true, include \p End in the range.
