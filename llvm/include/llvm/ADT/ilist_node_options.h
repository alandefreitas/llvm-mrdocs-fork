...
template <bool IsExplicit> struct explicitness {
  /// True when the corresponding option was written explicitly by the user.
  static const bool is_explicit = IsExplicit;
};
/// Explicitness marker for options written by the user.
using is_explicit = explicitness<true>;
/// Marker that an option value came from a default rather than user input.
...
struct node_options {
  ...
  /// True when nodes track whether they are list sentinels.
  static const bool enable_sentinel_tracking = EnableSentinelTracking;
  /// True when sentinel tracking was requested explicitly via an option.
  static const bool is_sentinel_tracking_explicit = IsSentinelTrackingExplicit;
  /// True when iterators carry extra open/half-open range bits.
  static const bool has_iterator_bits = HasIteratorBits;
  /// Tag type distinguishing this node from other lists of the same value type.
  using tag = TagT;
  /// Parent object type stored on nodes when ilist_parent is enabled.
  using parent_ty = ParentTy;
  /// Concrete ilist_node_base instantiation for these options.
  using node_base_type = ilist_node_base<enable_sentinel_tracking, parent_ty>;
  /// Concrete ilist_base instantiation for these options.
  using list_base_type = ilist_base<enable_sentinel_tracking, parent_ty>;
};

/// Compute \a node_options for value type \p T and the given \p Options.
template <class T, class... Options> struct compute_node_options {
  /// Resolved node_options for value type \p T and \p Options.
