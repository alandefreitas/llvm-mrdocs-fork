...
/// Arbitrary-precision integer. @seebelow
class APInt;
class hash_code;
...
public:
  /// Sentinel index value meaning "not found".
  ...
  /// Reverse iterator over characters in the string.
  using reverse_iterator = std::reverse_iterator<iterator>;
  /// Const reverse iterator over characters in the string.
...
#pragma clang diagnostic pop
  ...
  /// Construct a StringLiteral from a character array that may contain
  /// embedded NUL bytes, such as \c "foo\\0bar".
  ///
  /// Unlike the array constructor, the length is taken as \c N - 1 without
  /// requiring that the array be NUL-terminated at the end.
  /// @param Str Character array literal, including any embedded NULs.
  /// @returns A StringLiteral whose size is \c N - 1.
  template <size_t N>
