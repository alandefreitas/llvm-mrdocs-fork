...
public:
  /// Opaque identifier for a single change in the input set.
  using change_ty = unsigned;
  /// Directed dependency edge from the first change to the second.
  using edge_ty = std::pair<change_ty, change_ty>;

  // FIXME: Use a decent data structure.
  /// Set of change identifiers forming a candidate subset.
  using changeset_ty = std::set<change_ty>;
  /// Ordered collection of change sets used during search.
