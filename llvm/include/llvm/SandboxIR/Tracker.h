...
/// A class that saves hashes and textual IR snapshots of.
///
/// functions in a SandboxIR Context, and does hash comparison when `expectNoDiff` is called. If hashes differ, it prints textual IR for both old and new versions to aid debugging. This is used as an additional debug check when reverting changes to SandboxIR, to verify the reverted state matches the initial state.
...
///
/// The two template arguments are: - GetterFn: The getter member function pointer (e.g., `&Foo::get`) - SetterFn: The setter member function pointer (e.g., `&Foo::set`) Upon construction, it saves a copy of the original value by calling the getter function. Revert sets the value back to the one saved, using the setter function provided. Example: Tracker.track(std::make_unique< GenericSetter<&FooInst::get, &FooInst::set>>(I, Tracker));
