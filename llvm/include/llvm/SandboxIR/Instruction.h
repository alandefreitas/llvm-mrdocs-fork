...
public:
  ...
  ///
  /// source vector without changing the length of that vector. Example: shufflevector <4 x n> A, <4 x n> B, <3,0,undef,3>
  ...
  /// Return true if this shuffle chooses elements from exactly one.
  ///
  /// source vector without lane crossings and does not change the number of elements from its input vectors. Example: shufflevector <4 x n> A, <4 x n> B, <4,undef,6,undef>
  ...
  /// Return true if this shuffle mask chooses all elements with.
  ///
  /// the same value as the first element of exactly one source vector. Example: <4,undef,undef,4> This assumes that vector operands (of length \p NumSrcElts) are the same length as the mask.
  ...
  /// Return true if all elements of this shuffle are the.
  ///
  /// same value as the first element of exactly one source vector without changing the length of that vector. Example: shufflevector <4 x n> A, <4 x n> B, <undef,0,undef,0>
  ...
  ///
  /// concatenating the two inputs together and then extracts an original width vector starting from the splice index. Example: shufflevector <4 x n> A, <4 x n> B, <1,2,3,4> This assumes that vector operands (of length \p NumSrcElts) are the same length as the mask.
  ...
  ///
  /// each index between [0..VF) is used exactly once in each submask of size VF. For example, the mask for \p VF=4 is: 0, 1, 2, 3, 3, 2, 0, 1 - "clustered", because each submask of size 4 (0,1,2,3 and 3,2,0,1) uses indices [0..VF) exactly one time. 0, 1, 2, 3, 3, 3, 1, 0 - not "clustered", because element 3 is used twice in the second submask (3,3,1,0) and index 2 is not used at all.
...
public:
  ...
  ///
  /// Note that this operation may reorder the remaining cases at index idx and above. Note: This action invalidates iterators for all cases following the one removed, including the case_end() iterator. It returns an iterator for the next case.
...
/// An or instruction, which can be marked as "disjoint", indicating.
///
/// that the inputs don't have a 1 in the same bit position. Meaning this instruction can also be treated as an add.
