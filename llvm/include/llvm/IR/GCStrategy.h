...
///
/// Provides overridable hooks for those needs which cannot be abstractly
/// described.  GCStrategy objects must be looked up through the Function.
/// The objects themselves are owned by the Context and must be immutable.
class GCStrategy {
private:
...
protected:
  ...
  ///
  /// If set, NeededSafePoints and UsesMetadata should be left at their
  /// default values.
  bool UseStatepoints = false;

  /// Whether RewriteStatepointsForGC should rewrite calls when UseStatepoints
  /// is set.
  ///
  /// If UseStatepoints is set, this determines whether the
  /// RewriteStatepointsForGC pass should rewrite this function's calls.
...
public:
  ...
  /// @return The collector name string for this strategy.
  const std::string &getName() const { return Name; }

  /// Returns true if this strategy is expecting the use of gc.statepoints,
  /// and false otherwise.
  /// @return True if this strategy uses gc.statepoints.
  bool useStatepoints() const { return UseStatepoints; }

  /** @name Statepoint Specific Properties */
  ///@{

  /// Returns whether \p Ty is a pointer to a GC-managed location, if known.
  ///
  ...
  /// \param Ty The type to classify as GC-managed or not.
  /// @return True if \p Ty is GC-managed, false if not, or std::nullopt if
  ///         unknown.
  ...
  /// using this GC.
  /// @return True if RewriteStatepointsForGC should rewrite this strategy's
  ///         functions.
  bool useRS4GC() const {
  ...
  /// @return True if the backend must emit metadata tables.
  ...
  /// @return True if calls are inferred to be safepoints.
...
/// \param Name The garbage collector name string to look up.
/// @return A newly created GCStrategy for \p Name.
