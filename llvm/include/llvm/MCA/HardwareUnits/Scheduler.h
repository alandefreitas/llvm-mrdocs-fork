...
/// Abstract strategy used by the Scheduler to rank ready instructions.
class LLVM_ABI SchedulerStrategy {
public:
  /// Construct a scheduler strategy.
  SchedulerStrategy() = default;
  /// Destroy a scheduler strategy.
  ...
  /// @param Lhs Left-hand ready instruction candidate.
  /// @param Rhs Right-hand ready instruction candidate.
  /// @return True if \p Lhs should take priority over \p Rhs.
...
public:
  /// Construct the default scheduler strategy.
  DefaultSchedulerStrategy() = default;
  /// Destroy the default scheduler strategy.
  ~DefaultSchedulerStrategy() override;

  /// Compare two ready instructions by age and known user count.
  /// @param Lhs Left-hand ready instruction candidate.
  /// @param Rhs Right-hand ready instruction candidate.
  /// @return True if \p Lhs should take priority over \p Rhs.
...
public:
  /// Construct a Scheduler for \p Model using load/store unit \p Lsu.
  /// @param Model Machine scheduling model describing processor resources.
  /// @param Lsu Load/store unit used to track memory dependencies.
  Scheduler(const MCSchedModel &Model, LSUnitBase &Lsu)
      : Scheduler(Model, Lsu, nullptr) {}

  /// Construct a Scheduler with an optional instruction selection strategy.
  /// @param Model Machine scheduling model describing processor resources.
  /// @param Lsu Load/store unit used to track memory dependencies.
  /// @param SelectStrategy Selection strategy, or null to use the default.
  Scheduler(const MCSchedModel &Model, LSUnitBase &Lsu,
            std::unique_ptr<SchedulerStrategy> SelectStrategy)
      : Scheduler(std::make_unique<ResourceManager>(Model), Lsu,
                  std::move(SelectStrategy)) {}

  /// Construct a Scheduler that owns the given resource manager.
  /// @param RM Resource manager that tracks processor pipeline resources.
  /// @param Lsu Load/store unit used to track memory dependencies.
  /// @param SelectStrategy Selection strategy, or null to use the default.
  ...
  /// Stalls generated by the scheduler when checking dispatch availability.
  enum Status {
    /// Scheduler and LS resources are available for dispatch.
    SC_AVAILABLE,
    /// The load queue is full.
    SC_LOAD_QUEUE_FULL,
    /// The store queue is full.
    SC_STORE_QUEUE_FULL,
    /// Scheduler buffers are full.
    SC_BUFFERS_FULL,
    /// Dispatch stalled because the current dispatch group is incomplete.
  ...
  /// @return Availability status for dispatching \p IR.
  ...
  /// dependencies.
  /// @param IR Instruction to dispatch into the scheduler queues.
  /// @return True if \p IR is ready to issue to the underlying pipelines.
  LLVM_ABI bool dispatch(InstRef &IR);

  /// Issue an instruction to the underlying pipelines.
  ///
  /// Populates a vector of used pipeline resources, and a vector of
  /// instructions that transitioned to the ready state as a result of this
  /// event.
  /// @param IR Instruction to issue.
  /// @param Used Pipeline resources consumed by issuing \p IR.
  /// @param Pending Instructions that entered the pending state.
  /// @param Ready Instructions that became ready as a result of this issue.
  ...
  /// latency instruction.
  /// @param IR Instruction to check for immediate issue.
  /// @return True if \p IR must be issued immediately.
  ...
  /// @param Freed Resources that became available in this cycle.
  /// @param Executed Instructions that finished executing in this cycle.
  /// @param Pending Instructions that entered or left the pending state.
  /// @param Ready Instructions that became ready in this cycle.
  ...
  /// @param Mask Resource mask whose most significant bit identifies a resource.
  ...
  ///
  /// Returns an invalid instruction reference if there are no ready
  /// instructions, or if processor resources are not available.
  /// @return The selected instruction, or an invalid reference if none can issue.
  LLVM_ABI InstRef select();

  /// Return true if the ReadySet contains no instructions.
  /// @return True if ReadySet is empty.
  bool isReadySetEmpty() const { return ReadySet.empty(); }
  /// Return true if the WaitSet contains no instructions.
  /// @return True if WaitSet is empty.
  bool isWaitSetEmpty() const { return WaitSet.empty(); }

  /// Identify instructions stalled by unresolved data dependencies.
  ///
  ...
  /// @param RegDeps Instructions blocked on register dependencies.
  /// @param MemDeps Instructions blocked on memory dependencies.
  LLVM_ABI void analyzeDataDependencies(SmallVectorImpl<InstRef> &RegDeps,
                                        SmallVectorImpl<InstRef> &MemDeps);

  /// Return a mask of busy resources that blocked instruction issue.
  ///
  /// Also populates vector Insts with instructions that could not be issued to
  /// the underlying pipelines because not all pipeline resources were
  /// available.
  /// @param Insts Instructions that could not issue due to busy resources.
  /// @return Bitmask of busy resources that prevented issue.
  LLVM_ABI uint64_t analyzeResourcePressure(SmallVectorImpl<InstRef> &Insts);

  /// Return true if dispatch stalled on unavailable scheduler or LS resources.
  /// @return True if the previous stage stalled due to unavailable resources.
  bool hadTokenStall() const { return HadTokenStall; }

#ifndef NDEBUG
  /// Dump the contents of the scheduler ready queues for debugging.
  void dump() const;

  /// Assert that \p IR is not already present in any scheduler queue.
  ///
  ...
  /// queues.
  /// @param IR Instruction that must not already be queued.
