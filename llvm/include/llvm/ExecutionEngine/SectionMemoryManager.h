...
public:
  ...
  enum class AllocationPurpose {
    ...
    ROData,
    /// Allocate memory for read-write data.
    RWData,
  };
  ...
  public:
    /// Allocate virtual memory for the given purpose.
    ///
    ...
    ///
    /// \param Purpose Reason for the allocation (code, RO data, or RW data).
    /// \param NumBytes Number of bytes of virtual memory to allocate.
    /// \param NearBlock Optional existing block to try to allocate near.
    /// \param Flags Initial protection flags for the allocated memory.
    /// \param EC [out] Receives an error code if allocation fails.
    /// \returns A non-null MemoryBlock on success, otherwise a null MemoryBlock
    /// with \p EC describing the error.
    ...
    /// Set protection flags for a previously mapped memory block.
    ///
    ...
    ///
    /// \param Block Memory block whose protection flags are updated.
  ...
  /// \param MM Memory mapper to use, or nullptr for the default OS mapper.
  /// \param ReserveAlloc Whether to pre-allocate all memory up front.
  ...
  SectionMemoryManager(const SectionMemoryManager &Unused) = delete;
  /// Deleted copy assignment operator.
  /// \param Unused Ignored; copy assignment is not supported.
  void operator=(const SectionMemoryManager &Unused) = delete;
  /// Destroy the section memory manager.
  ~SectionMemoryManager() override;

  /// Enable reserveAllocationSpace when requested.
  /// \returns True if allocation space should be reserved up front.
  bool needsToReserveAllocationSpace() override { return ReserveAllocation; }

  /// Allocate all section memory in a single contiguous block.
  ///
  /// This is required to limit memory offsets to fit the ARM ABI; large memory
  /// systems may otherwise allocate separate sections too far apart.
  ...
  /// \returns Pointer to the allocated code section memory.
  ...
  /// \returns Pointer to the allocated data section memory.
