...
/// Alias of \c std::once_flag used with \c llvm::call_once.
using once_flag = std::once_flag;
  ...
  public:
    ...
    // higher).
    /// Suggested upper bound on worker threads; 0 means use all available.
    unsigned ThreadsRequested = 0;

    /// If SMT is active, use hyper threads; otherwise one std::thread per core.
    bool UseHyperThreads = true;

    /// When true, clamp ThreadsRequested to hardware thread or core counts.
    bool Limit = false;
  ...
  enum class SetThreadPriorityResult {
    /// The requested priority could not be applied.
    FAILURE,
    /// The requested priority was applied successfully.
    SUCCESS
  };
  /// Set the calling thread's priority to \p Priority.
  ///
  /// \param Priority Desired scheduling priority.
  /// \return Whether the platform accepted the change.
