...
struct CachePruningPolicy {
  ...
  ///
  /// This is intended to be used to avoid scanning the directory too often. It
  /// does not impact the decision of which file to prune. A value of 0 forces
  /// the scan to occur. A value of std::nullopt disables pruning.
  std::optional<std::chrono::seconds> Interval = std::chrono::seconds(1200);

  /// The expiration for a file.
  ///
  /// When a file hasn't been accessed for Expiration seconds, it is removed
  /// from the cache. A value of 0 disables the expiration-based pruning.
  std::chrono::seconds Expiration = std::chrono::hours(7 * 24); // 1w

  /// The maximum size for the cache directory as a percentage of available
  /// disk space.
  ///
  /// Set to 100 to indicate no limit, 50 to indicate that the cache size will
  /// not be left over half the available disk space. A value over 100 will be
  /// reduced to 100. A value of 0 disables the percentage size-based pruning.
  ...
  /// the amount of available space. A value of 0 disables the absolute
  /// size-based pruning.
...
///
/// Defaults are taken from a default constructed CachePruningPolicy object.
...
///
/// \param PolicyStr String describing the pruning policy to parse.
/// \return The parsed CachePruningPolicy, or an Error if PolicyStr is invalid.
...
/// pattern "llvmcache-*".
///
/// \param Path Cache directory to prune.
/// \param Policy Pruning policy to apply.
/// \param Files Optional buffers from the current job, used to warn when the
///        job exceeds size or file-count limits.
/// \return True if pruning occurred, or an Error on failure.
