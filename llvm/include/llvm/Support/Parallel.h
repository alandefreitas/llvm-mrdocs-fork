...
///
/// Defaults to using all hardware threads and should be initialized before the
/// first use of parallel routines.
...
public:
  /// Create a task group that may spawn work onto the parallel executor.
  LLVM_ABI TaskGroup();
  /// Wait for outstanding tasks and destroy the group.
  LLVM_ABI ~TaskGroup();

  /// Enqueue callable \p f without waiting for it to finish.
  ///
  /// \param f Work item to run asynchronously when parallelism is enabled.
...
/// Sort the half-open range [\p Start, \p End) in parallel when enabled.
///
/// \param Start Beginning of the random-access range.
/// \param End End of the random-access range.
/// \param Comp Strict-weak-ordering comparator; defaults to \c std::less.
...
/// Invoke \p Fn for each index in [\p Begin, \p End), possibly in parallel.
///
/// \param Begin First index to process.
/// \param End One-past-last index to process.
/// \param Fn Callable invoked with each index.
LLVM_ABI void parallelFor(size_t Begin, size_t End,
                          function_ref<void(size_t)> Fn);

/// Apply \p Fn to each element in [\p Begin, \p End), possibly in parallel.
///
/// \param Begin Beginning of the random-access range.
/// \param End End of the random-access range.
/// \param Fn Callable invoked once per element.
template <class IterTy, class FuncTy>
void parallelForEach(IterTy Begin, IterTy End, FuncTy Fn) {
  parallelFor(0, End - Begin, [&](size_t I) { Fn(Begin[I]); });
}

/// Transform each element in [\p Begin, \p End) and reduce the results in parallel.
///
...
// Range wrappers.
/// Sort range \p R in parallel when enabled.
///
/// \param R Random-access range to sort.
/// \param Comp Strict-weak-ordering comparator; defaults to \c std::less.
...
/// Apply \p Fn to each element of range \p R, possibly in parallel.
///
...
/// Transform each element of \p R and reduce the results in parallel.
///
/// \param R Input range.
/// \param Init Initial value for the reduction.
/// \param Reduce Binary reducer combining partial results.
...
/// Apply \p Fn to each element of \p R in parallel and join any returned Errors.
///
/// \param R Range whose elements are visited.
