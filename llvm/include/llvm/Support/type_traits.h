...
public:
  /// True if \c T is an integral or enumeration type.
...
template <typename T> struct add_lvalue_reference_if_not_pointer {
  /// Reference or pointer type corresponding to \c T.
...
template <typename T> struct add_const_past_pointer {
  /// Const-qualified pointer or reference type corresponding to \c T.
  using type = std::conditional_t<std::is_pointer_v<T>,
                                  const std::remove_pointer_t<T> *, const T>;
};

/// Choose const pointer or const reference based on whether \c T is a pointer.
/// Choose const pointer or const reference based on whether \c T is a pointer.
template <typename T> struct const_pointer_or_const_ref {
  /// Const pointer if \c T is a pointer; otherwise const reference.
  using type =
