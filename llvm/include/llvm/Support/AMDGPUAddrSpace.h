...
enum : unsigned {
  /// Highest address-space id among flat/generic/local/private/constant/region.
...
enum class FlatAddrSpace : unsigned {
  FLAT,        ///< Generic flat address space.
  FlatGlobal,  ///< Flat global address space segment.
  FlatScratch, ///< Flat scratch address space segment.
};
...
enum : unsigned {
  GLOBAL = 0,       ///< DWARF global address space.
  GENERIC = 1,      ///< DWARF generic address space.
  REGION = 2,       ///< DWARF region address space.
  LOCAL = 3,        ///< DWARF local address space.
  PRIVATE_LANE = 5, ///< DWARF private per-lane address space.
  PRIVATE_WAVE = 6, ///< DWARF private per-wave address space.
  DEFAULT = GLOBAL, ///< Default DWARF address space.
};
} // namespace DWARFAS

/// Implementation details for AMDGPU address-space mapping.
namespace impl {
