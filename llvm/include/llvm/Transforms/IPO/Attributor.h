...
/// Return true if \p V is dynamically unique, that is,.
///
/// there are no two "instances" of \p V at runtime with different values. Note: If \p ForAnalysisOnly is set we only check that the Attributor will never use \p V to represent two "instances" not that \p V could not technically represent them.
...
/// Return true if the value of \p VAC is a.
///
/// valid at the position of \p VAC, that is a constant, an argument of the same function, or an instruction in that function that dominates the position.
...
/// Return true if \p ToI is potentially reachable from \p.
///
/// FromI without running into any instruction in \p ExclusionSet The two instructions do not need to be in the same function. \p GoBackwardsCB can be provided to convey domain knowledge about the "lifespan" the user is interested in. By default, the callers of \p FromI are checked as well to determine if \p ToI can be reached. If the query is not interested in callers beyond a certain point, e.g., a GPU kernel entry or the function containing an alloca, the \p GoBackwardsCB should return false.
...
struct IRPosition {
  ...
  ///
  /// If \p IRP is a call site (see isAnyCallSitePosition()) then the result will be a call site position, otherwise the function position of the associated function.
  ...
  /// Return the callee argument number of the associated value if.
  ///
  /// it is an argument or call site argument, otherwise a negative value. In contrast to `getCallSiteArgNo` this method will always return the "argument number" from the perspective of the callee. This may not the same as the call site if this is a callback call.
...
struct Attributor {
  ...
  ///
  /// Return true if there is any change. The flag \p ChangeDroppable indicates if dropppable uses should be changed too.
  ...
  /// Return the attributes of any kind in \p AKs existing in the IR at a position that will affect this one.
  ///
  /// While each position can only have a single attribute of any kind in \p AKs, there are "subsuming" positions that could have an attribute as well. This method returns all attributes found in \p Attrs. \param IgnoreSubsumingPositions Flag to determine if subsuming positions, e.g., the function position if this is an argument position, should be ignored.
  /// \param IgnoreSubsumingPositions Flag to determine if subsuming positions,
...
public:
  ...
  /// Return \p std::nullopt if there is no call back registered.
  ///
  /// for \p GV or the call back is still not sure if \p GV can be simplified. Return \p nullptr if \p GV can't be simplified.
...
public:
  ...
  /// Helper struct used in the communication between an abstract attribute.
  ///
  /// (AA) that wants to change the signature of a function and the Attributor which applies the changes. The struct is partially initialized with the information from the AA (see the constructor). All other members are provided by the Attributor prior to invoking any callbacks.
...
/// Abstract attribute for the no-unwind function attribute.
...
struct AAPointerInfo : public AbstractAttribute {
  ...
  /// Call \p CB on all accesses that might interfere with.
  ///
  /// \p Range and return true if all such accesses were known and the callback returned true for all of them, false otherwise. An access interferes with an offset-size pair if it might read or write that memory region.
