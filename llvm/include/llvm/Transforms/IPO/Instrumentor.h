...
struct InstrumentationOpportunity {
  ...
  /// Compute the opportunity identifier for the current instrumentation epoch \p CurrentEpoch.
  ///
  /// The identifiers are assigned consecutively as the epoch advances. Epochs may have no identifier assigned (e.g., because no id was requested). This function always returns the same identifier when called multiple times with the same epoch.
