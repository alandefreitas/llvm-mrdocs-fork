...
struct SectionDescriptorBase {
  /// Construct a section descriptor.
  /// \param SectionKind Kind of debug section described.
  /// \param Format DWARF form parameters used by the section.
  /// \param Endianess Endianness used by the section.
  ...
  /// \return The endianness used by the section.
  llvm::endianness getEndianess() const { return Endianess; }
  /// Returns FormParams used by section.
  /// \return The DWARF form parameters used by the section.
...
protected:
  ...
  /// Endianness used by the section.
  llvm::endianness Endianess = llvm::endianness::little;
};

/// Callback invoked for each produced output DWARF section.
...
public:
  ...
  /// \return A new DWARFLinker instance.
  LLVM_ABI static std::unique_ptr<DWARFLinker>
  createLinker(MessageHandlerTy ErrorHandler, MessageHandlerTy WarningHandler);

  /// Set the output DWARF handler.
  ///
  /// Result of linking DWARF is set of sections containing final debug info.
  /// DWARFLinkerBase::link() pass generated sections using specified
  /// \p SectionHandler.
  /// \param TargetTriple Target triple for the output DWARF.
  /// \param SectionHandler Callback invoked for each generated section.
